...
@synthesize tableViewShow, textCustomTitle, textOfDate;
...
- (void)viewDidLoad
{
    ...
    [self setCustomTitleOther:textCustomTitle];
    
    [super viewDidLoad];
        
    tableViewShow.separatorStyle = UITableViewCellSeparatorStyleNone;
    tableViewShow.backgroundColor = [FlightTool colorFromHexString:@"f3f3f3"];
    self.tableViewShow.contentInset = UIEdgeInsetsMake(-64, 0, 0, 0);
	...
	NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://202.55.191.108:8084/flighttime/information?date=%@", textOfDate]]];
    ...
    [icao_list setObject:@"Сөүл Пусан" forKey:@"RKSS"];
    ...
    [icao_list setObject:@"Красноярск"forKey:@"UNKL"];
    ...
    [icao_list setObject:@"Хөх хот" forKey:@"ZBHH"];
    ...
    ODRefreshControl * refreshControl = [[ODRefreshControl alloc] initInScrollView:self.tableViewShow];
...
- (void)tableView: (UITableView *)tableView didSelectRowAtIndexPath: (NSIndexPath *)indexPath
{
    ...
    flightController.textCurrentDate = textCustomTitle;
...
- (NSString * ) returnStatus: (NSString *) stat
{
    ...
    if ([stat isEqualToString:@"DELAYED"]) {
        returnStatusText = @"Нислэг саатсан";
    }
    if ([stat isEqualToString:@"CNL"]) {
        returnStatusText = @"Нислэг цуцлагдсан";
    }
